...
$(window).on('load', function(){
	...
	function username_filter(username){
        ...
        meetsReqs = false;
    	if (username.length >= 8 && username.length <= 25) { //ensure correct length
	...
	orangeForm_pass.on('keyup',function(){
		var strongRegex = new RegExp("/^(?=.*[A-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[$@])(?!.*[iIoO])\S{6,12}$/");
		var mediumRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");
		var enoughRegex = new RegExp("(?=.{6,8}).*", "g");
		switch(true){
			case orangeForm_pass.val().length==0: 
				orangeForm_pass.siblings('label').text('Type your password').attr('class','red-text'); break;
			case enoughRegex.test(orangeForm_pass.val())==true:
				orangeForm_pass.siblings('label').text('Weak Password').attr('class','orange-text'); break;
			case mediumRegex.test(orangeForm_pass.val())==true:
				orangeForm_pass.siblings('label').text('Medium Strength').attr('class','yellow-text'); break;
			case strongRegex.test(orangeForm_pass.val())==true:
